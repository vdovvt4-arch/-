...
if (window.Auth && window.Api) {
  ...
  window.addEventListener("firebase-ready", guardAdmin, { once: true });
}
        ...
        <div class="admin-card-header">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div>
              ...
              <div style="font-size:.8rem;color:var(--ink-soft);">${s.lectureCount} محاضرة · ${s.materialCount} ملزمة</div>
            </div>
          </div>
          <div class="admin-card-actions">
...
// Sub-tabs
document.querySelectorAll(".admin-tab[data-subtab]").forEach(btn => {
  btn.addEventListener("click", () => {
    document.querySelectorAll(".admin-tab[data-subtab]").forEach(b => b.classList.remove("active"));
    document.querySelectorAll("#panel-lectures .admin-subtab").forEach(t => t.classList.remove("active"));
    btn.classList.add("active");
...
document.getElementById("addLecBtn").addEventListener("click", async () => {
  ...
  if (!url && !file) { showToast("يرجى إضافة رابط يوتيوب أو رفع ملف فيديو", "error"); return; }
  ...
  finally { btn.disabled = false; btn.textContent = "➕ رفع المحاضرة"; }
});
...
document.getElementById("addMatBtn").addEventListener("click", async () => {
  ...
  finally { btn.disabled = false; btn.textContent = "➕ رفع الملف"; }
});
  ...
  try {
    ...
    list.innerHTML = users.map(u => `
      <div class="admin-card">
        ...
        <div style="font-size:.82rem;color:var(--ink-soft);margin-top:4px;">
          ${esc(u.email || "")}
        </div>
      </div>
    `).join("");
...
// ── Confirm Dialog (native confirm — no extra modal markup needed) ──

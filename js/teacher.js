...
if (window.Auth && window.Api) {
  ...
  window.addEventListener("firebase-ready", guardTeacher, { once: true });
}
...
document.getElementById("publishLecBtn").addEventListener("click", async () => {
  ...
  finally { btn.disabled = false; btn.textContent = "🚀 نشر الفيديو الآن"; }
});
...
document.getElementById("publishMatBtn").addEventListener("click", async () => {
  ...
  finally { btn.disabled = false; btn.textContent = "🚀 رفع الملف الآن"; }
});

...
// Sends (or resends) the Firebase verification email. The link takes the
// student back to whatever domain the site is currently running on, so
// this works the same on the Firebase Hosting URL and on a custom domain.
...
const Auth = {
  async signUpWithEmail(email, password, fullName, username) {
    try {
      ...
      try { await sendVerification(cred.user); } catch (e) { /* account still created, non-fatal */ }
  ...
  // Blocks sign-in until the email is verified. If it isn't, we sign the
  // user back out (so they can't slip into the app unverified) and, best
  // effort, fire off a fresh verification link so someone who registered
  // before this check existed — or who lost the first email — isn't
  // permanently stuck.
  async signInWithEmail(email, password) {
    try {
      const cred = await signInWithEmailAndPassword(auth, email, password);
      if (!cred.user.emailVerified) {
        try { await sendVerification(cred.user); } catch (e) { /* rate-limited etc., ignore */ }
        await signOut(auth);
        return {
          data: null,
          error: {
            code: "app/email-not-verified",
            message: "لم يتم تفعيل بريدك الإلكتروني بعد. أرسلنا لك رابط تفعيل جديد — تحقق من صندوق الوارد أو الرسائل غير المرغوب فيها."
          }
        };
      }
  ...
  // Manual "resend" action for the login screen: signs in just long enough
  // to trigger a fresh verification email, then signs back out.
  async resendVerification(email, password) {
    try {
      const cred = await signInWithEmailAndPassword(auth, email, password);
      if (cred.user.emailVerified) {
        await signOut(auth);
        return { error: { code: "app/already-verified", message: "بريدك الإلكتروني مُفعّل بالفعل — جرّب تسجيل الدخول" } };
      }
